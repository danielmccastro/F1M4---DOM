...
function listarAnimais() { // Implementação da função (10 pontos)
  ...
  if (select_animais.value == "dog") {
    for (let i = 0; i <= cachorros.length; i++) {
      let imgCachorro = document.createElement("img");
      imgCachorro.src = "img/dog" + (i + 1) + ".jpg";
      imgCachorro.id = cachorros[i].nome;
      container.appendChild(imgCachorro);
    }
  }
  if (select_animais.value == "cat") {
    for (let i = 0; i <= gatos.length; i++) {
      let imgGato = document.createElement("img");
      imgGato.src = "img/cat" + (i + 1) + ".jpg";
      imgGato.id = gatos[i].nome;
      container.appendChild(imgGato);
    }
  }
}
